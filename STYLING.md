...
'?' denotes what direction the piece will move in, for now we will just keep symbols that move in a single direction, as all other moves can be constructed from them: - '>' right - '<' left - 'v' backwards - '^' forwards - '/^' up-right - '\\^' up-left - '/v' down-left - '\\v' down-right
