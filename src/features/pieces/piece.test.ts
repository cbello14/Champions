...
describe("Piece", () => {
	describe("addMove", () => {
		...
		test("preserves existing moves", () => {
			const piece = new Piece("p", "", [moveA], ["direct"]);
	...
	describe("removeMoveAt", () => {
		test("removes the move at the given index", () => {
			const piece = new Piece("p", "", [moveA, moveB], ["direct", "no-capture"]);
	...
	describe("addMovement", () => {
		test("appends a movement to the move at the given index", () => {
			const piece = new Piece("p", "", [{ ...moveA, movements: [{ direction: moveDirection.up, distance: 1 }] }], ["direct"]);
		...
		test("preserves the piece id", () => {
			const piece = new Piece("p", "", [{ ...moveA, movements: [{ direction: moveDirection.up, distance: 1 }] }], ["direct"]);
			expect(piece.addMovement(0, { direction: moveDirection.right, distance: 1 }).id).toBe(piece.id);
		});
	});
	...
	describe("removeMovementAt", () => {
		...
		test("preserves the piece id", () => {
			const piece = new Piece("p", "", [{
				attributes: { type: moveMovementType.slide },
				movements: [{ direction: moveDirection.up, distance: 1 }, { direction: moveDirection.right, distance: 1 }]
			}], ["direct"]);
	...
	describe("toJSON", () => {
		test("serializes the piece fields correctly", () => {
			const piece = new Piece("rook", "rook.png", [moveA], ["direct"]);
			const json = piece.toJSON();
			expect(json.id).toBe(piece.id);
			expect(json.name).toBe("rook");
			expect(json.image).toBe("rook.png");
	...
	describe("fromJSON", () => {
		test("reconstructs a piece from its JSON", () => {
			...
			expect(restored.image).toBe(piece.image);
