import { Board, checkCoordinateEquality, coordinateToString, coordinateStringToCoordinate, checkCoordinateArrayIncludes } from "@/features/boards/board";
...
describe("checkCoordinateArrayIncludes", () => {
	test("returns true when the coordinate is present", () => {
		expect(checkCoordinateArrayIncludes([[1, 2], [3, 4]], [3, 4])).toBe(true);
	});
	test("returns false when the coordinate is absent", () => {
		expect(checkCoordinateArrayIncludes([[1, 2], [3, 4]], [5, 6])).toBe(false);
	});
